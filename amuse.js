...
class Amuse {
  async constructor(src, id, meta, load) {
    if (src && id) {
  ...
  async addMuse(src, id = false, meta = {}, load = true) {
    if (!id) {
      throw new Error("No id given to Muse constructor");
    } else if (this.getMuseById(id)) {
      throw new Error(
        "Used Id given to Muse constructor. Unique muse id is required."
      );
  ...
  async removeMuseById(id) {
    ...
    if (isNaN(museIndexToRemove))
      throw new Error("Muse id doesnt exist, cannot remove the muse");
  ...
  async load(id) {
    ...
    if (!this.getMuseById(id)) {
      ...
      if (!muse.audio) {
        ...
        for (e in this._audioEventListeners) {
          muse.audio.addEventListener(e, this._audioEventListeners[e]);
        }
  ...
  addEventListener(type, callback) {
    if (!this._eventListeners[type]) {
      throw new Error(
        "This type of event listener is not supported. Choose from " +
          Object.keys(this._eventListeners).join(" | ")
      );
  ...
  removeEventListener(type, callback) {
    if (!this._eventListeners[type]) {
      ...
      if (i < 0) throw new Error("No such eventListener exists.");
      else this._eventListeners[type].splice(i, 1);
    }
  }
  ...
  once(type, callback) {
    this.on(type, function (amuse) {
      callback(amuse);
      amuse.notOn(type, this);
    });
  }

  addSeekBar(outer, inner, t = 0) {
    if (!inner || !outer)
      throw new Error("No elements to the .addSeekBar() method");
    else {
      this._elements.bar.push({ inner, outer });
      onmousehold(
        outer,
        (e, x, y) => {
          //seek the bar when you hold it
          ...
          if (percentage >= 0 && percentage <= 1) {
            // check if the mouse is insed the bar
  ...
  addElement(type, el) {
    if (!this._elements[type]) {
      throw new Error(
        "This type of element is not supported. Choose from " +
          Object.keys(this._elements).join(" | ")
      );
    } else {
  ...
  set currentSeconds(s) {
    if (s > this.duration || s < 0)
      throw new Error("Can't seek to time out of the muse's range.");
    else {
      this.load().then(() => {
  ...
  get loaded() {
    return this.currentMuse ? (this.currentMuse.audio ? true : false) : false;
  }
  ...
  _on(event, ...args) {
    this._eventListeners[event].forEach((fn) => fn(this.currentMuse, ...args));
  }
  ...
  _updateElements() {
    ...
    let currentTime = {
      min: Math.floor(this.currentTimerentSeconds / 60),
      sec: Math.floor(this.currentTimerentSeconds % 60),
    };
    currentTime.sec < 10
      ? (currentTime.sec = "0" + currentTime.sec)
      : undefined;
    ...
    this._elements.currentTimerentTime.forEach((el) => {
      el.innerText = `${currentTime.min}:${currentTime.sec}`;
    });
    ...
    this._elements.completeTime.forEach((el) => {
      el.innerText = `${currentTime.min}:${currentTime.sec} / ${dur.min}:${dur.sec}`;
    });
  }
}
